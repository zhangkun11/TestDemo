...
public class AccessConditionTool extends BasicActivity {
     ...
     * using {@link BaseApplication#acBytesToACMatrix(byte[])},
     * {@link #acBitsToACRowNr(byte[], boolean)},
     * {@link #getResourceForDataBlocksByRowNr(int)} and
     * {@link #getResourceForSectorTrailersByRowNr(int)}.
     * @param view The View object that triggered the method
     * (in this case the decode button).
     * @see BaseApplication#acBytesToACMatrix(byte[])
    ...
    public void onDecode(View view) {
        ...
        byte[][] acMatrix = BaseApplication.acBytesToACMatrix(
                BaseApplication.hexStringToByteArray(ac));
        boolean error = false;
        if (acMatrix != null) {
     ...
     * {@link BaseApplication#acMatrixToACBytes(byte[][])} and display them.
     * @param view The View object that triggered the method
     * (in this case the encode button).
     * @see BaseApplication#acMatrixToACBytes(byte[][])
     * @see #mACMatrix
     * @see #acRowNrToACBits(int, boolean)
     */
    public void onEncode(View view) {
        mAC.setText(BaseApplication.byte2HexString(BaseApplication.acMatrixToACBytes(mACMatrix)));
    }
    ...
    public void onCopyToClipboard(View view) {
        BaseApplication.copyToClipboard(mAC.getText().toString(), this);
    }
    ...
    public void onPasteFromClipboard(View view) {
        String text = BaseApplication.getFromClipboard(this);
        if (text != null) {
            mAC.setText(text);
        }
    }
