 ...
 * retrieved by calling {@link BaseApplication#getKeyMap()}.</li>
...
//            "de.syss.MifareClassicTool.Activity.KEY_MAP";
    ...
    public void onStart() {
        ...
        // setting is active).
        boolean selectLastUsedKeyFiles = BaseApplication.getPreferences().getBoolean(
     ...
     * {@link BaseApplication#setKeyMap(android.util.SparseArray)}.
    ...
    public void onCreateKeyMap(View view) {
        boolean saveLastUsedKeyFiles = BaseApplication.getPreferences().getBoolean(
                Preference.SaveLastUsedKeyFiles.toString(), true);
        ...
        if (fileNames.size() > 0) {
            ...
            if (keyFiles.size() > 0) {
                ...
                BaseApplication.setKeyMapRange(mFirstSector, mLastSector);
                // Init. GUI elements.
                mProgressStatus = -1;
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (mIsCreatingKeyMap == true
                            ...
                            BaseApplication.setKeyMap(null);
                            BaseApplication.setKeyMapRange(-1, -1);
    ...
    private void keyMapCreated(MCReader reader) {
        ...
        if (reader.getKeyMap().size() == 0) {
            ...
            BaseApplication.setKeyMap(reader.getKeyMap());
