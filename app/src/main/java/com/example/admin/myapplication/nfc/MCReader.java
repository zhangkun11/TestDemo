...
import com.nfc.Preferences.Preference;
...
public class MCReader {
        ...
        if (auth) {
            ...
            for (int i = firstBlock; i < lastBlock; i++) {
                try {
                    ...
                    blocks.add(BaseApplication.byte2HexString(blockBytes));
                } catch (TagLostException e) {
                    throw e;
                } catch (IOException e) {
            ...
            if (!useAsKeyB) {
                if (isKeyBReadable(BaseApplication.hexStringToByteArray(
                        ret[last].substring(12, 20)))) {
                    ret[last] = BaseApplication.byte2HexString(key)
                ...
                if (ret[0].equals(NO_DATA)) {
                    ...
                    ret[last] = NO_KEY + ret[last].substring(12, 20)
                            + BaseApplication.byte2HexString(key);
                }
            }
        }
        return ret;
    ...
    public int buildNextKeyMapPart() {
        ...
        if (mKeysWithOrder != null && mLastSector != -1) {
            ...
            boolean autoReconnect = BaseApplication.getPreferences().getBoolean(
     ...
     * ({@link BaseApplication#getOperationInfoForBlock(byte, byte, byte,
     * de.syss.MifareClassicTool.BaseApplication.Operations, boolean, boolean)}) of the
     * Access Conditions.
    ...
    public HashMap<Integer, HashMap<Integer, Integer>> isWritableOnPositions(
        ...
        for (int i = 0; i < keyMap.size(); i++) {
            ...
            if (pos.containsKey(sector)) {
                ...
                byte[][] acMatrix = BaseApplication.acBytesToACMatrix(ac);
                if (acMatrix == null) {
                    ret.put(sector, null);
                    continue;
                }
                ...
                for (int block : pos.get(sector)) {
                    if ((block == 3 && sector <= 31)
                        ...
                        int acValue = BaseApplication.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                BaseApplication.Operations.WriteAC,
                                true, isKeyBReadable);
                        // Is key A writable? (If so, key B will be writable
                        // with the same key.)
                        int keyABValue = BaseApplication.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                BaseApplication.Operations.WriteKeyA,
                                true, isKeyBReadable);
                        ...
                        blockWithWriteInfo.put(
                                block, BaseApplication.getOperationInfoForBlock(
                                        acMatrix[0][acBitsForBlock],
                                        acMatrix[1][acBitsForBlock],
                                        acMatrix[2][acBitsForBlock],
                                        BaseApplication.Operations.Write,
    ...
    public boolean setKeyFile(File[] keyFiles, Context context) {
        ...
        for (File file : keyFiles) {
            String[] lines = BaseApplication.readFileLineByLine(file, false, context);
            if (lines != null) {
                for (String line : lines) {
                    if (!line.equals("") && line.length() == 12
                            && line.matches("[0-9A-Fa-f]+")) {
                        try {
                            keys.add(BaseApplication.hexStringToByteArray(line));
