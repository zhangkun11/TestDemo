...
public class TagInfoTool extends BasicActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateTagInfos(BaseApplication.getTag());
    }

    /**
     * Calls {@link BaseApplication#treatAsNewTag(Intent, android.content.Context)} and
     * then calls {@link #updateTagInfos(Tag)}
     */
    @Override
    public void onNewIntent(Intent intent) {
        BaseApplication.treatAsNewTag(intent, this);
    ...
    private void updateTagInfos(Tag tag) {
        ...
        if (tag != null) {
            ...
            mMFCSupport = BaseApplication.checkMifareClassicSupport(tag, this);

            mLayout.removeAllViews();
            // Display generic info.
            // Create views and add them to the layout.
            TextView headerGenericInfo = new TextView(this);
            headerGenericInfo.setText(BaseApplication.colorString(
            ...
            String uid = BaseApplication.byte2HexString(tag.getId());
            ...
            String atqa = BaseApplication.byte2HexString(atqaBytes);
            String sak = BaseApplication.byte2HexString(
                    new byte[] {(byte)nfca.getSak()});
            String ats = "-";
            IsoDep iso = IsoDep.get(tag);
            if (iso != null ) {
                ats = BaseApplication.byte2HexString(iso.getHistoricalBytes());
            }
            // Identify tag type.
            ...
            genericInfo.setText(TextUtils.concat(
                    BaseApplication.colorString(getString(R.string.text_uid) + ":", hc),
                    "\n", uid, "\n",
                    BaseApplication.colorString(getString(
                            R.string.text_rf_tech) + ":", hc),
                    // Tech is always ISO 14443a due to NFC Intet filter.
                    "\n", getString(R.string.text_rf_tech_14a), "\n",
                    BaseApplication.colorString(getString(R.string.text_atqa) + ":", hc),
                    "\n", atqa, "\n",
                    BaseApplication.colorString(getString(R.string.text_sak) + ":", hc),
                    "\n", sak, "\n",
                    BaseApplication.colorString(getString(
            ...
            if (mMFCSupport == 0) {
                ...
                headerMifareInfo.setText(BaseApplication.colorString(
