...
public class ValueBlockTool extends BasicActivity {
    ...
    public void onDecode(View view) {
        ...
        if (BaseApplication.isValueBlock(data) == false) {
        ...
        // Decode.
        byte[] vbAsBytes = BaseApplication.hexStringToByteArray(
                data.substring(0, 8));
        // Bytes -> Int. -> reverse.
    ...
    public void onEncode(View view) {
        ...
        String vb = BaseApplication.byte2HexString(ByteBuffer.allocate(4).putInt(
                Integer.reverseBytes(vbAsInt)).array());
        // Int. -> invert -> reverse -> byte array -> hex string.
        String vbInverted = BaseApplication.byte2HexString(ByteBuffer.allocate(4).putInt(
                Integer.reverseBytes(~vbAsInt)).array());
    ...
    public void onCopyToClipboard(View view) {
        BaseApplication.copyToClipboard(mVB.getText().toString(), this);
    }
