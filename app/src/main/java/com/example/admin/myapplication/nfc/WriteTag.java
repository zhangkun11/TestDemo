...
public class WriteTag extends BasicActivity {
    ...
    private void createKeyMapForBlock(int sector) {
        ...
        intent.putExtra(KeyMapCreator.EXTRA_KEYS_DIR,
                Environment.getExternalStoragePublicDirectory(BaseApplication.HOME_DIR)
    ...
    private void writeBlock() {
        MCReader reader = BaseApplication.checkForTagAndCreateReader(this);
        if (reader == null) {
            return;
        }
        int sector = Integer.parseInt(mSectorText.getText().toString());
        int block = Integer.parseInt(mBlockText.getText().toString());
        byte[][] keys = BaseApplication.getKeyMap().get(sector);
        int result = -1;

        if (keys[1] != null) {
            result = reader.writeBlock(sector, block,
                    BaseApplication.hexStringToByteArray(mDataText.getText().toString()),
                    keys[1], true);
        }
    ...
    private void readDumpFromFile(String pathToDump) {
        ...
        String[] dump = BaseApplication.readFileLineByLine(file, false, this);
    ...
    private void checkDumpAndShowSectorChooserDialog(String[] dump) {
        int err = BaseApplication.isValidDump(dump, false);
        if (err != 0) {
            // Error.
            BaseApplication.isValidDumpErrorToast(err, this);
            return;
        }
        ...
        for (int i = 0; i < dump.length; i++) {
            if (dump[i].startsWith("+")) {
                ...
                mDumpWithPos.get(sector).put(block++,
                        BaseApplication.hexStringToByteArray(dump[i]));
            } else {
                block++;
            }
        }
     ...
     * @see BaseApplication#getOperationInfoForBlock(byte, byte,
     * byte, de.syss.MifareClassicTool.BaseApplication.Operations, boolean, boolean)
    ...
    private void createFactoryFormatedDump() {
        ...
        // So BaseApplication.getTag() will return den current present tag
        // (and its size/sector count).
        mDumpWithPos = new HashMap<Integer, HashMap<Integer,byte[]>>();
        int sectors = MifareClassic.get(BaseApplication.getTag()).getSectorCount();
